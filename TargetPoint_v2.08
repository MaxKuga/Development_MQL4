...
// ⛔️ DAILY LOSS LIMIT — глобальный стоп торговли на день
   ...
   if(!CanTradeToday())
   {
      if(lastLossLimitLog < today && Debug) Print("[DAILY-LOSS-LIMIT] Trading blocked for today. losses=",
                      CountLosingTradesToday(), " limit=", MaxLosingTradesPerDay);
      lastLossLimitLog = today;
      CancelAllPendings(); // снимаем висячие ордера
   ...
   double currentLot = NormalizeDouble(BaseLot * MathPow(1.5, LossSeries), 2);
   if (currentLot > MarketInfo(Symbol(), MODE_MAXLOT)) currentLot = MarketInfo(Symbol(), MODE_MAXLOT);
